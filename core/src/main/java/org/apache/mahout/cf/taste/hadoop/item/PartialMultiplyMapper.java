...
public final class PartialMultiplyMapper extends
  ...
  protected void map(VarIntWritable key,
    ...
    Vector similarityMatrixColumn = vectorAndPrefsWritable.getVector();//跟当前item相似的item列表
    List<Long> userIDs = vectorAndPrefsWritable.getUserIDs();//对当前item评分过的user
    List<Float> prefValues = vectorAndPrefsWritable.getValues();

    for (int i = 0; i < userIDs.size(); i++) {
      long userID = userIDs.get(i);
      float prefValue = prefValues.get(i);
      if (!Float.isNaN(prefValue)) {//有分数才放进去
        prefAndSimilarityColumn.set(prefValue, similarityMatrixColumn);
        userIDWritable.set(userID);
        context.write(userIDWritable, prefAndSimilarityColumn);//输出: user,  (pref,simirarities)
      }
    }
  }
